...
export function readDesktopConfig(root: string): DesktopProjectConfig {
  ...
  if (config?.app || config?.desktop) {
    const desktop = config.desktop ?? {};
    const { permissions, updater, ...window } = desktop;
    return {
      app: config.app,
      window,
      permissions: permissions as DesktopPermissions | undefined,
      updater,
    };
  }
  const legacy = resolve(root, "onestack.desktop.json");
  return existsSync(legacy) ? readJson(legacy) as DesktopProjectConfig : {};
}
...
function copyWebDist(root: string, destination: string) {
  ...
  for (const entry of ["index.html", "assets"]) {
    ...
    if (existsSync(source)) cpSync(source, resolve(destination, entry), { recursive: true });
  }
}

function stagePortable(root: string, outputRoot: string, nativeTarget: string, manifest: DesktopManifest) {
  const binary = resolve(outputRoot, packagedBinaryName(manifest));
  copyFileSync(nativeTarget, binary);
...
function stageMac(root: string, outputRoot: string, nativeTarget: string, manifest: DesktopManifest) {
  ...
  mkdirSync(resolve(resources, ".onestack"), { recursive: true });
  const binary = resolve(macos, packagedBinaryName(manifest));
  copyFileSync(nativeTarget, binary);
...
function toolExists(command: string, args = ["--version"]) {
  try { return spawnSync(command, args, { stdio: "ignore", shell: process.platform === "win32" }).status === 0; }
  catch { return false; }
}
...
function packageDesktop(outputRoot: string, manifest: DesktopManifest, strict: boolean) {
  ...
  if (process.platform === "linux") {
    ...
    if (toolExists("dpkg-deb")) { run("dpkg-deb", ["--build", debRoot, deb]); artifacts.push(deb); }
    ...
    if (toolExists("appimagetool")) {
      const appDir = mkdtempSync(resolve(tmpdir(), "onestack-appdir-"));
      const libDir = resolve(appDir, "usr/lib", appSlug);
      mkdirSync(libDir, { recursive: true });
      cpSync(outputRoot, libDir, { recursive: true });
    ...
    artifacts.push(resolve(outputRoot, packagedBinaryName(manifest)));
    return artifacts;
  }
  if (process.platform === "win32") {
    const exe = resolve(outputRoot, packagedBinaryName(manifest));
    artifacts.push(exe);
...
function signDesktop(outputRoot: string, manifest: DesktopManifest, artifacts: string[], strict: boolean) {
  ...
  if (process.platform === "darwin") {
    ...
    const dmg = artifacts.find((item) => item.endsWith(".dmg"));
  ...
  if (process.platform === "win32") {
    ...
    for (const artifact of artifacts.filter((item) => /\.(?:exe|msi)$/i.test(item))) {
