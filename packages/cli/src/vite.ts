...
export function secureViteArgs(root: string, args: string[]) {
  ...
  const origin = publicApiOrigin(root);
  const backend = new URL('./backend.js', import.meta.url).href;
  const security = new URL('./security.js', import.meta.url).href;
  writeFileSync(output, `import { devApiPlugin } from ${JSON.stringify(backend)};\nimport { loadConfigFromFile, mergeConfig } from 'vite';\nimport { isServerOnly } from ${JSON.stringify(security)};\nexport default async env => {\n const loaded = ${config ? `await loadConfigFromFile(env, ${JSON.stringify(config)})` : 'null'};\n return mergeConfig(loaded?.config ?? {}, { define: { __ONESTACK_API_ORIGIN__: ${JSON.stringify(JSON.stringify(origin))} }, plugins: [devApiPlugin(${JSON.stringify(root)}), { name: 'onestack-server-boundary', enforce: 'pre', resolveId(id, importer, options) { if (!options?.ssr && isServerOnly(id)) throw new Error('Server-only import in client: ' + id); }, transform(code, id, options) { if (!options?.ssr && isServerOnly(id)) throw new Error('Server-only client module: ' + id); } }] });\n};\n`);
